//
//  WKCCircleProgressView.m
//  Almighty
//
//  Created by 魏昆超 on 2018/11/29.
//  Copyright © 2018 SecretLisa. All rights reserved.
//

#import "WKCCircleProgressView.h"

#define WKCCircleDegreeToRadian(d) ((d)*M_PI)/180.0
#define WKCCircleSelfWidth self.frame.size.width
#define WKCCircleSelfHeight self.frame.size.height

@interface WKCCircleProgressView()
...
@implementation WKCCircleProgressView

- (instancetype)init
{
    if (self = [super init]) {
        [self slc_initialization];
    }
    return self;
}
                  ...
                  strokeWidth:(CGFloat)strokeWidth
{
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        _endColor = endColor;
        _startAngle = WKCCircleDegreeToRadian(startAngle);
        _reduceAngle = WKCCircleDegreeToRadian(reduceAngle);
...
- (void)slc_initialization
{
    ...
    _textFont = [UIFont systemFontOfSize:0.15*WKCCircleSelfWidth];
    
    _strokeWidth = 10;
    _subdivCount = 64;
    _animationDuration = 2;
    _radius = MIN(CGRectGetWidth(self.frame), CGRectGetHeight(self.frame))/2.0;
    _startAngle = -WKCCircleDegreeToRadian(90);
    _reduceAngle = WKCCircleDegreeToRadian(0);
...
-(void)drawRect:(CGRect)rect
{
    ...
    [self slc_drawGradientInContext:ctx
...
- (void)slc_drawGradientInContext:(CGContextRef)ctx
                    startingAngle:(float)startAngle
                       ...
                       withCenter:(CGPoint)center
{
    ...
    p0 = [self slc_pointForTrapezoidWithAngle:currentAngle
                                    andRadius:intRadius
                                    forCenter:center];
    p3 = [self slc_pointForTrapezoidWithAngle:currentAngle
                                    andRadius:outRadius
                                    forCenter:center];
    ...
    for (int i = 0;i < subdivCount; i++) {
        ...
        UIColor *currentColor = [self slc_getGradientColor:colorFraction];
        
        currentAngle = startAngle + fraction*(endAngle - startAngle);
        CGMutablePathRef trapezoid = CGPathCreateMutable();
        
        p1 = [self slc_pointForTrapezoidWithAngle:currentAngle + angleDelta
                                        andRadius:intRadius
                                        forCenter:center];
        p2 = [self slc_pointForTrapezoidWithAngle:currentAngle + angleDelta
                                        andRadius:outRadius
                                        forCenter:center];
        ...
        if (_showProgressText) {
            ...
            //垂直居中
            CGRect r = CGRectMake((int)((WKCCircleSelfWidth-stringSize.width)/2.0), (int)((WKCCircleSelfHeight - stringSize.height)/2.0),(int)stringSize.width, (int)stringSize.height);
...
- (void)setProgress:(CGFloat)progress
{
    ...
    if (_notAnimated) {
        ...
        CADisplayLink *playLink = [CADisplayLink displayLinkWithTarget:self selector:@selector(slc_countingAction)];
...
//获取当前颜色
- (UIColor *)slc_getGradientColor:(CGFloat)current
{
    CGFloat c1[4];
    CGFloat c2[4];
...
//获取点
- (CGPoint)slc_pointForTrapezoidWithAngle:(float)a
                                andRadius:(float)r
                                forCenter:(CGPoint)p
{
...
{
    if (_startAngle != WKCCircleDegreeToRadian(startAngle)) {
        _startAngle = WKCCircleDegreeToRadian(startAngle);
        [self setNeedsDisplay];
    }
}
...
{
    if (_reduceAngle != WKCCircleDegreeToRadian(reduceAngle)) {
        if (reduceAngle>=360) {
            return;
        }
        _reduceAngle = WKCCircleDegreeToRadian(reduceAngle);
